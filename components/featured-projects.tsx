            ...
            <div className="space-y-6">
                {projects.map((project, index) => (
                    <Link
                        ...
                        target="_blank"
                        className="group flex gap-6 items-start hover:opacity-80 transition-opacity"
                    >
                        <div className="relative w-[180px] h-[110px] rounded-md overflow-hidden bg-zinc-100 border border-zinc-200 shrink-0">
                            <Image
                                src={project.image}
                                alt={project.title}
                                fill
                        ...
                        <div className="flex flex-col py-1">
                            <div className="flex items-center gap-2 mb-1">
                                <h3 className="font-bold text-base text-zinc-900 leading-none">
                                    {project.title}
                                </h3>
                                <span className="text-zinc-400 text-base font-light leading-none">|</span>
                                <span className="text-sm font-medium text-zinc-800 leading-none">
                                    {project.tagline}
                                </span>
                            </div>

                            <p className="text-sm text-zinc-500 mb-2 leading-relaxed line-clamp-2 max-w-[480px]">
                                {project.description}
                            </p>

                            <div className="flex items-center gap-2 text-zinc-600 group-hover:text-zinc-900 transition-colors">
                                <div className="w-4 h-4 rounded-sm bg-zinc-100 border border-zinc-200 flex items-center justify-center overflow-hidden">
                                    <span className="text-[8px] font-bold uppercase">{project.icon}</span>
                                </div>
